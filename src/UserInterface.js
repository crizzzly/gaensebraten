...
class UserInterface extends Component {
  ...
  itemStyle(item) {
    return Object.assign({}, item.style, {
      transform: `translate3d(${item.pos[0]}px, ${item.pos[1]}px, ${item.pos[2]}px)`
    })
  }
    ...
    if (hands && hands.length){
      ...
      hands.forEach((hand, index) => {
        if(hand.type === 'left'){
          ...
          leftThumb = this.getFingerScreenPosition(hand.indexFinger.dipPosition)
          ...
          rightThumb = this.getFingerScreenPosition(hand.indexFinger.dipPosition)
